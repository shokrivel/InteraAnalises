...
interface PageContent {
  ...
  conteudo_texto: string; url_imagem?: string;
}
interface SaibaMaisCard {
  id: string; titulo: string; descricao?: string;
  url_imagem: string; ordem: number; ativo: boolean;
}
    ...
    const fetchData = async () => {
      try {
        ...
        const { data: pageData, error: pageError } = await supabase
          .from('paginas_conteudo').select('*').eq('slug', 'saiba-mais').maybeSingle();
        if (pageError && pageError.code !== 'PGRST116') { setError('Erro ao carregar.'); return; }
        if (pageData) setContent(pageData);
        const { data: cardsData } = await (supabase as any)
          .from('saiba_mais_cards').select('*').eq('ativo', true).order('ordem', { ascending: true });
        setCards(cardsData || []);
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <Card><CardContent className="pt-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-4">{error || 'Conteúdo não encontrado.'}</p>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle className="text-3xl font-bold text-center">{content.titulo}</CardTitle>
            </CardHeader>
            <CardContent className="prose prose-lg max-w-none">
              {content.url_imagem && (
                <div className="mb-8 text-center">
                  <img src={content.url_imagem} alt="Imagem da página" className="mx-auto rounded-lg shadow-md max-w-full h-auto"/>
                </div>
              )}
                  ...
                  <div key={index}>
                    {paragraph.startsWith('•') ? (
                      <ul className="list-disc list-inside space-y-2 ml-4">
                        {paragraph.split('\n').map((item, i) => (
                          <li key={i} className="text-muted-foreground">{item.replace('•','').trim()}</li>
                        ))}
                      </ul>
                    ) : (
                ...
                {cards.map(card => (
                  <Card key={card.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    <div className="aspect-video w-full overflow-hidden">
                      <img src={card.url_imagem} alt={card.titulo} className="w-full h-full object-cover"
                        onError={e => { e.currentTarget.src = "/placeholder.svg"; }}/>
                    </div>
                    <CardContent className="p-4">
                      <h3 className="font-semibold text-lg mb-2">{card.titulo}</h3>
                      {card.descricao && <p className="text-muted-foreground text-sm">{card.descricao}</p>}
                    </CardContent>
                  </Card>
                ))}
              </div>
