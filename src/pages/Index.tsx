...
import iaBg from './ia-homepage-bg.png';
...
export default function Index() {
  ...
  // ── Cores e fontes exatas da imagem ─────────────────────────────────
  const BLUE   = '#3b4da0';  // azul-roxo do texto Intera/Analises
  const DARK   = '#2d2d4e';  // texto escuro dos labels
  const GRAY   = '#e8e9ea';  // fundo cinza direito
  const STAR   = '#FF8A65';  // estrela laranja
  ...
  const panel = () => {
    switch (open) {
      ...
      case 'auth': return (
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10, marginTop:22 }}>
            <button style={T.btnFill} onClick={() => { setAuthOpen(true); setOpen(null); }}>
              Entrar na conta
            </button>
            <button style={T.btnOutline} onClick={() => { setAuthOpen(true); setOpen(null); }}>
              Criar conta gratis
            </button>
          </div>
        </div>
      ...
      // ── NOTICIAS — direciona para /noticias ──────────────────────────
      case 'news': return (
        <div>
      ...
      case 'newsletter': return (
        <div>
          ...
          <p style={T.psub}>Receba novidades, atualizacoes e conteudos exclusivos diretamente no seu e-mail.</p>
          {subDone ? (
            <div style={{ marginTop:20, background:'rgba(59,77,160,0.08)', border:`1px solid ${BLUE}`, borderRadius:12, padding:18, textAlign:'center' }}>
              ...
              <div style={{ fontSize:12, color:'#888', marginTop:4 }}>Voce esta na lista. Obrigado!</div>
            </div>
          ) : (
            <form onSubmit={subscribe} style={{ marginTop:20, display:'flex', flexDirection:'column', gap:10 }}>
              <label style={T.plbl}>Seu e-mail</label>
              <input
                type="email" required placeholder="voce@email.com"
      ...
      // ── QUEM SOMOS — placeholder ate o usuario estruturar ────────────
      case 'about': return (
        <div>
          ...
          <div style={{ background:'rgba(59,77,160,0.06)', borderRadius:12, padding:'14px 16px', fontSize:13, color:DARK, lineHeight:1.7 }}>
            ...
            Tornar o diagnostico medico mais acessivel, preciso e centrado no paciente — com tecnologia de ponta e responsabilidade clinica.
          </div>
        </div>
      );

      // ── CONTATO — placeholder ate o usuario estruturar ──────────────
      case 'contact': return (
        <div>
          ...
          {contactDone ? (
            ...
            <form onSubmit={sendContact} style={{ marginTop:18, display:'flex', flexDirection:'column', gap:10 }}>
              <div>
                <label style={T.plbl}>Nome</label>
                <input type="text" required placeholder="Seu nome" value={contactForm.name}
                  onChange={e => setContactForm(p => ({...p, name:e.target.value}))} style={T.pinp}/>
              </div>
              <div>
                <label style={T.plbl}>E-mail</label>
                <input type="email" required placeholder="voce@email.com" value={contactForm.email}
                  onChange={e => setContactForm(p => ({...p, email:e.target.value}))} style={T.pinp}/>
              </div>
              <div>
                <label style={T.plbl}>Mensagem</label>
                <textarea required rows={4} value={contactForm.msg}
                  onChange={e => setContactForm(p => ({...p, msg:e.target.value}))}
                  style={{ ...T.pinp, resize:'none', height:88 }}/>
              </div>
              <button type="submit" disabled={contactSending}
                style={{ ...T.btnFill, opacity: contactSending ? .7 : 1 }}>
                {contactSending ? 'Enviando...' : 'Enviar mensagem'}
              </button>
            </form>
          )}
          <div style={{ marginTop:16, fontSize:12, color:'#999', lineHeight:1.8 }}>
            contato@interaanalises.com.br
          </div>
        </div>
      );
  ...
  return (
    <div style={{
      ...
      {/* ═══ ESQUERDA — ia-homepage-bg.png como fundo ═══ */}
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>

        {/* A propria imagem do Canva ocupa 100% */}
        <img
          src={iaBg}
          alt=""
          style={{
        ...
        {/* Textos sobrepostos — posicao exata da imagem */}
        <div style={{
          position: 'absolute',
          top: '38%', left: '18%',
      ...
      <div style={{
        ...
        {/* Lista de pills — largura fixa 370px */}
        <div style={{
          width: 370,
          flexShrink: 0,
          display: 'flex',
          ...
          {MENU.map(item => (
            ...
            >
              {/* Estrela laranja — asset MAEWRAI7B-M do Canva */}
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" style={{ flexShrink:0 }}>
                <path d="M12 2C10.5 7 7 10.5 2 12c5 1.5 8.5 5 10 10 1.5-5 5-8.5 10-10-5-1.5-8.5-5-10-10z" fill={STAR} />
              </svg>
              {/* Label */}
              <span style={{ flex:1, fontSize:14, fontWeight:600, color:DARK, letterSpacing:'0.1px' }}>
                {item.label}
              </span>
              {/* Seta > — asset MADWz0YNEmk do Canva */}
              <svg width="7" height="11" viewBox="0 0 7 12" fill="none" style={{ flexShrink:0 }}>
                <path d="M1 1l5 5-5 5"
                  stroke={open === item.id ? BLUE : '#bbb'}
        ...
        {/* Painel de conteudo — aparece ao expandir */}
        {open && (
          <div style={{
            flex: 1,
            overflowY: 'auto',
...
const BLUE = '#3b4da0';
const T = {
  ptitle:   { fontSize:18, fontWeight:700, color:'#2d2d4e', margin:'0 0 8px' } as React.CSSProperties,
  ...
  pinp:     { width:'100%', background:'#fff', border:'1px solid #ddd', borderRadius:8, padding:'9px 12px', fontSize:13, color:'#2d2d4e', outline:'none', fontFamily:"'Inter',sans-serif" } as React.CSSProperties,
