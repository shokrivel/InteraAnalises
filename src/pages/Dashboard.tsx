  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f0e8', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{
        ...
        <div
          style={{ fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: '1.3rem', color: '#0f4a2e', cursor: 'pointer', letterSpacing: '-0.5px' }}
          onClick={() => navigate('/')}
        >
          Intera<span style={{ color: '#c8a84b' }}>Análises</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ background: '#e8f5ee', color: '#1a7a4a', fontSize: '0.78rem', fontWeight: 600, padding: '5px 12px', borderRadius: 2 }}>
            {user?.email}
          </div>
          {isAdmin && (
            <button onClick={() => navigate('/admin')} style={navBtn('#0f4a2e', '#fafaf8')}>
      ...
      <div style={{ background: '#0f4a2e', padding: '64px 60px 56px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.68rem', letterSpacing: 3, textTransform: 'uppercase', color: '#c8a84b', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ width: 28, height: 1, background: '#c8a84b', display: 'inline-block' }} />
            Painel do Usuário
          </div>
          ...
          <p style={{ color: 'rgba(255,255,255,.6)', fontSize: '1rem', maxWidth: 520 }}>
            Sua plataforma para consultas personalizadas em Parasitologia, Bioquímica e Hematologia.
          </p>
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
          {cards.map(({ icon: Icon, title, desc, btn, action, primary }) => (
            ...
            >
              ...
              <button
                onClick={action}
                style={primary ? btnPrimary : btnOutline}
                onMouseEnter={e => Object.assign(e.currentTarget.style, primary ? btnPrimaryHover : btnOutlineHover)}
          ...
          {isAdmin && (
            <div style={dashCard}
              ...
              onMouseLeave={e => Object.assign(e.currentTarget.style, dashCard)}
            >
              <div style={{ width: 52, height: 52, background: 'rgba(200,168,75,.15)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
              ...
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.2rem', fontWeight: 700, marginBottom: 8, color: '#1c1c1c' }}>
                Painel Admin
                <span style={{ marginLeft: 8, background: '#e8f5ee', color: '#1a7a4a', fontSize: '0.65rem', fontWeight: 700, padding: '3px 7px', borderRadius: 2, verticalAlign: 'middle' }}>ADMIN</span>
              </h3>
              ...
              <button
                ...
                style={btnGold}
                onMouseEnter={e => Object.assign(e.currentTarget.style, btnGoldHover)}
      ...
      <footer style={{ background: '#1c1c1c', padding: '28px 60px', textAlign: 'center', fontSize: '0.78rem', color: 'rgba(255,255,255,.4)' }}>
...
// ── style objects ──────────────────────────────────────────────
const navBtn = (bg: string, fg: string): React.CSSProperties => ({
...
const dashCardHover: React.CSSProperties = {
  ...dashCard,
  borderColor: '#1a7a4a',
  borderTopColor: '#c8a84b',
  boxShadow: '0 8px 32px rgba(26,122,74,.1)',
...
const btnPrimary: React.CSSProperties = {
  ...
  borderRadius: 2, border: 'none', cursor: 'pointer', transition: 'background .2s',
};
const btnPrimaryHover: React.CSSProperties = { ...btnPrimary, background: '#1a7a4a' };
const btnOutline: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', gap: 8,
  background: 'transparent', color: '#0f4a2e',
  border: '2px solid #0f4a2e', fontWeight: 700,
  fontSize: '0.8rem', letterSpacing: 1, textTransform: 'uppercase',
  padding: '10px 20px', borderRadius: 2, cursor: 'pointer', transition: 'all .2s',
};
const btnOutlineHover: React.CSSProperties = { ...btnOutline, background: '#e8f5ee' };
const btnGold: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', gap: 8,
  background: '#c8a84b', color: '#0f4a2e',
  fontWeight: 700, fontSize: '0.8rem', letterSpacing: 1,
  textTransform: 'uppercase', padding: '12px 22px',
  borderRadius: 2, border: 'none', cursor: 'pointer', transition: 'background .2s',
};
const btnGoldHover: React.CSSProperties = { ...btnGold, background: '#e8c96a' };
