...
import { ArrowLeft, Bot, MapPin, Star, Navigation, Map, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
const ConsultationChat = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!s?.access_token) throw new Error('Sessao expirada.');
        ...
        if (!data?.response) throw new Error('Nao foi possivel processar a consulta.');
        setRes(data);
  ...
  const profileLabels: Record<string, string> = {
    patient: 'Paciente', academic: 'Academico', health_professional: 'Profissional de Saude',
  };
  const filtered = specialists.filter(s => s.rating >= ratingFilter);

  if (loading) return (
    <div style={{ minHeight:'100vh', background:'#f4fbfa', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Inter',sans-serif" }}>
      <div style={{ textAlign:'center', padding:32 }}>
        <div style={{ width:64, height:64, background:TEAL_LIGHT, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 24px', animation:'pulse 1.5s ease-in-out infinite' }}>
          <Bot size={30} color={TEAL} />
        </div>
        <h2 style={{ fontWeight:700, fontSize:19, color:SLATE, marginBottom:8 }}>Analisando suas informacoes...</h2>
        <p style={{ fontSize:13.5, color:'#6b7280', marginBottom:24, maxWidth:320 }}>
          A IA esta processando sua consulta e identificando os melhores especialistas.
        </p>
        <div style={{ display:'flex', gap:6, justifyContent:'center' }}>
          {[0,0.15,0.3].map((d,i) => (
            <div key={i} style={{ width:9, height:9, borderRadius:'50%', background:TEAL, animation:`bounce .9s ${d}s ease-in-out infinite` }}/>
          ))}
        </div>
      </div>
      <style>{`@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.07)}}@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}`}</style>
    </div>
  );

  if (!res) return (
    <div style={{ minHeight:'100vh', background:'#f4fbfa', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ textAlign:'center', padding:40 }}>
        <p style={{ color:'#6b7280', marginBottom:16 }}>Nao foi possivel processar a consulta.</p>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f4fbfa', fontFamily:"'Inter',sans-serif" }}>
      ...
      {/* NAV */}
      <nav style={{ background:'#fff', borderBottom:`1px solid ${TEAL}33`, padding:'0 24px', height:60, display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:50 }}>
        ...
        {profile && (
          <span style={{ background:TEAL_LIGHT, color:TEAL_DARK, fontSize:12, fontWeight:600, padding:'4px 12px', borderRadius:20 }}>
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'32px 20px 64px' }}>
        ...
        {/* Consulta concluida */}
        <div style={{ background:'#fff', borderRadius:16, border:`1px solid ${TEAL}33`, padding:'28px 28px 24px', marginBottom:20, textAlign:'center' }}>
          <div style={{ width:56, height:56, borderRadius:'50%', background:TEAL_LIGHT, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
            <CheckCircle2 size={28} color={TEAL}/>
          </div>
          <h2 style={{ fontWeight:700, fontSize:20, color:SLATE, marginBottom:8 }}>Consulta concluida</h2>
          <p style={{ fontSize:14, color:'#6b7280', lineHeight:1.65, maxWidth:420, margin:'0 auto 20px' }}>
            Suas informacoes foram analisadas. Com base nos dados,
            recomendamos um <strong style={{ color:TEAL_DARK }}>{specialtyHint}</strong> para avaliacao.
          </p>
          {res.suggestedSpecialty && (
            <span style={{ display:'inline-block', background:TEAL_LIGHT, color:TEAL_DARK, fontSize:13, fontWeight:600, padding:'6px 16px', borderRadius:20 }}>
        ...
        {/* Especialistas */}
        {filtered.length > 0 && (
          <div style={{ background:'#fff', borderRadius:14, border:`1px solid ${TEAL}33`, overflow:'hidden', marginBottom:16 }}>
            <div style={{ padding:'14px 20px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10 }}>
              <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                ...
                <span style={{ fontWeight:600, fontSize:14, color:SLATE }}>Profissionais recomendados proximos</span>
              </div>
            ...
            {filtered.map((sp,i) => (
              <div key={sp.placeId}
                style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'14px 20px', borderBottom:i<filtered.length-1?'1px solid #f3f4f6':'none', background:i===0?TEAL_LIGHT:'#fff' }}
                onMouseEnter={e=>{if(i>0)e.currentTarget.style.background='#f9fafb';}}
                onMouseLeave={e=>{if(i>0)e.currentTarget.style.background='#fff';}}
              >
                <div style={{ width:30, height:30, borderRadius:9, background:i===0?TEAL:'#f3f4f6', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:12, fontWeight:700, color:i===0?'#fff':'#6b7280' }}>{i+1}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:3 }}>
                    <p style={{ fontWeight:600, fontSize:14, color:SLATE }}>{sp.name}</p>
                    {i===0&&<span style={{ background:TEAL_LIGHT, color:TEAL_DARK, fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20 }}>Mais proximo</span>}
                  </div>
                ...
                <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                  <button onClick={()=>window.open(`https://www.google.com/maps/place/?q=place_id:${sp.placeId}`,'_blank')} style={iconBtn} title="Ver no Maps"><MapPin size={13}/></button>
                  <button onClick={()=>window.open(`https://www.google.com/maps/dir/?api=1&destination=${sp.location.lat},${sp.location.lng}`,'_blank')} style={{ ...iconBtn, background:TEAL, color:'#fff', border:'none' }} title="Como chegar"><Navigation size={13}/></button>
                </div>
              </div>
            ))}
          </div>
        ...
        {showMap&&specialists.length>0&&(
          <div style={{ background:'#fff', borderRadius:14, border:`1px solid ${TEAL}33`, overflow:'hidden', marginBottom:16 }}>
            <div style={{ padding:'14px 20px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontWeight:600, fontSize:14, color:SLATE }}>Mapa interativo</span>
        ...
        <div style={{ background:'#fff8ec', border:'1px solid #fcd34d', borderRadius:10, padding:'12px 16px' }}>
          <p style={{ fontSize:12.5, color:'#92400e', lineHeight:1.7 }}>
            ...
            <strong>Importante:</strong> Esta analise e gerada por IA para orientacao e nao substitui avaliacao medica profissional.
          </p>
        </div>
      </div>
    </div>
...
const btnPrimary: React.CSSProperties = { background:TEAL, color:'#fff', fontWeight:600, fontSize:13.5, padding:'9px 20px', borderRadius:8, border:'none', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:6 };
