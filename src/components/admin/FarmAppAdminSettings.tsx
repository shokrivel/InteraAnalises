/**
 * FarmAppAdminSettings.tsx
 * Componente para o painel admin do Vercel (InteraAnalises/AdminPanel)
 * Aba "FarmApp" — gerencia: valor da teleconsulta, atendentes, fila, config.
 *
 * Adicionar no AdminPanel.tsx:
 *   import FarmAppAdminSettings from "@/components/admin/FarmAppAdminSettings";
 *   <TabsTrigger value="farmapp">FarmApp</TabsTrigger>
 *   <TabsContent value="farmapp"><FarmAppAdminSettings /></TabsContent>
 *
 * Adicionar em .env.local:
 *   VITE_FARMAPP_SUPABASE_ANON_KEY=eyJ...
 */
...
import{Save,RefreshCw,Users,DollarSign,MessageSquare,Settings2,Plus,UserCheck}from "lucide-react";
import{useToast}from "@/hooks/use-toast";
import{createClient}from "@supabase/supabase-js";

const FARMAPP_URL ="https://rickfgexujrxbfcyfcfw.supabase.co";
const FARMAPP_KEY = import.meta.env.VITE_FARMAPP_SUPABASE_ANON_KEY||""
const farmapp = createClient(FARMAPP_URL, FARMAPP_KEY);
...
export default function FarmAppAdminSettings(){
  ...
  const loadAll=async()=>{
    ...
    const[s,a,q]=await Promise.all([
      farmapp.rpc("get_public_settings"),
      farmapp.from("chat_attendants").select("*").order("created_at"),
      farmapp.rpc("get_chat_queue"),
    ]);
  ...
  const saveSetting=async(key:string,value:string)=>{
    setSaving(key);
    const{error}=await farmapp.rpc("admin_set_setting",{p_key:key,p_value:value});
    setSaving(null);
  ...
  const saveMultiple=async(keys:string[])=>{
    setLoading(true);
    for(const k of keys)await farmapp.rpc("admin_set_setting",{p_key:k,p_value:settings[k]||""});
    setLoading(false);
  ...
  const addAttendant=async()=>{
    ...
    const{error}=await farmapp.rpc("admin_create_attendant",{p_display_name:newAtt.name,p_email:newAtt.email,p_role:newAtt.role});
    if(error){toast({title:"Erro",description:error.message,variant:"destructive"});}
  ...
  const toggleAttendant=async(a:Attendant)=>{
    await farmapp.rpc("admin_update_attendant",{p_id:a.id,p_display_name:a.display_name,p_email:a.email,p_role:a.role,p_is_active:!a.is_active});
    loadAll();
  };

  const closeSession=async(id:string)=>{
    if(!confirm("Finalizar este atendimento?"))return;
    await farmapp.rpc("close_chat_session",{p_session_id:id});
  ...
  if(loading&&!Object.keys(settings).length){
    return<div className="flex items-center justify-center p-12"><RefreshCw className="w-6 h-6 animate-spin text-primary"/><span className="ml-3 text-muted-foreground">Carregando dados do FarmApp\u2026</span></div>;
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="pricing">
        ...
        <TabsContent value="pricing" className="space-y-4 mt-4">
          <Card><CardHeader><CardTitle>Valor da teleconsulta</CardTitle>
            <CardDescription>Fixado aqui e exibido diretamente no app \u2014 n\u00e3o pode ser alterado pelo farmac\u00eautico.</CardDescription>
          </CardHeader><CardContent className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Valor cobrado do paciente (R$)</Label>
                <div className="flex gap-2">
                  <Input type="number" step="0.01" min="0" value={settings["telefarmacia_price"]||""} onChange={e=>set("telefarmacia_price")(e.target.value)}/>
                  <Button onClick={()=>saveSetting("telefarmacia_price",settings["telefarmacia_price"]||"0")} disabled={saving==="telefarmacia_price"}>
                    {saving==="telefarmacia_price"?<RefreshCw className="w-4 h-4 animate-spin"/>:<Save className="w-4 h-4"/>}
                  </Button>
                </div>
              </div>
              <div className="space-y-2">
                <Label>Comiss\u00e3o do farmac\u00eautico (%)</Label>
                <div className="flex gap-2">
                  <Input type="number" step="1" min="0" max="100" value={settings["pharmacist_commission"]||""} onChange={e=>set("pharmacist_commission")(e.target.value)}/>
                  <Button onClick={()=>saveSetting("pharmacist_commission",settings["pharmacist_commission"]||"70")} disabled={saving==="pharmacist_commission"}>
                    {saving==="pharmacist_commission"?<RefreshCw className="w-4 h-4 animate-spin"/>:<Save className="w-4 h-4"/>}
                  </Button>
                </div>
              </div>
            </div>
        ...
        <TabsContent value="attendants" className="space-y-4 mt-4">
          <Card><CardHeader className="flex flex-row items-center justify-between">
            <div><CardTitle>Funcion\u00e1rios do chat</CardTitle><CardDescription>Gerenciam o atendimento de an\u00e1lise de prescri\u00e7\u00f5es</CardDescription></div>
            <Button size="sm" onClick={()=>setShowAdd(v=>!v)}><Plus className="w-4 h-4 mr-1.5"/>Adicionar</Button>
            ...
            {showAdd&&(
              <div className="border rounded-xl p-4 space-y-3 bg-muted/40">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="space-y-1"><Label className="text-xs">E-mail *</Label><Input type="email" placeholder="ana@farmapp.com" value={newAtt.email} onChange={e=>setNewAtt(p=>({...p,email:e.target.value}))}/></div>
                  <div className="space-y-1"><Label className="text-xs">Cargo</Label>
                    <Select value={newAtt.role} onValueChange={v=>setNewAtt(p=>({...p,role:v}))}>
                      <SelectTrigger><SelectValue/></SelectTrigger>
                      <SelectContent><SelectItem value="attendant">Atendente</SelectItem><SelectItem value="supervisor">Supervisor</SelectItem></SelectContent>
                    </Select>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" onClick={addAttendant}>Cadastrar</Button>
                  <Button size="sm" variant="outline" onClick={()=>setShowAdd(false)}>Cancelar</Button>
                </div>
              </div>
            )}
            {!attendants.length?(
              <p className="text-center text-muted-foreground py-8">Nenhum funcion\u00e1rio cadastrado ainda.</p>
            ):(
              <div className="space-y-2">
                {attendants.map(a=>(
                  <div key={a.id} className={`flex items-center gap-3 p-3 rounded-lg border ${!a.is_active?"opacity-50":""}` }>
                    <div className="relative">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-lg font-bold text-primary">{a.display_name[0].toUpperCase()}</div>
                      {a.is_online&&a.is_active&&<span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-white"/>}
                    </div>
                    ...
                    <Switch checked={a.is_active} onCheckedChange={()=>toggleAttendant(a)} title={a.is_active?"Desativar":"Ativar"}/>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-2">
                {queue.map((q,i)=>(
                  <div key={q.session_id} className={`flex items-center gap-3 p-3 rounded-lg border ${q.status==="open"?"border-amber-300 bg-amber-50":"border-blue-200 bg-blue-50"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm">{q.patient_name||"Paciente"}</p>
                      <p className="text-xs text-muted-foreground truncate">{q.session_title}</p>
                    </div>
        ...
        <TabsContent value="settings" className="space-y-4 mt-4">
          ...
          <CardContent className="space-y-5">
            <div className="space-y-2">
              <Label>Aviso exibido no app (topo)</Label>
              <Input placeholder="Deixe vazio para n\u00e3o exibir" value={settings["app_notice"]||""} onChange={e=>set("app_notice")(e.target.value)}/>
              <p className="text-xs text-muted-foreground">Aparece como banner no dashboard do paciente</p>
            </div>
            <div className="space-y-2">
              <Label>Dom\u00ednio Jitsi (videochamadas)</Label>
              <Input value={settings["jitsi_domain"]||"meet.jit.si"} onChange={e=>set("jitsi_domain")(e.target.value)}/>
            </div>
            <div className="space-y-2">
            ...
            <div className="flex items-center justify-between py-3 border rounded-lg px-4">
              <div><p className="font-medium text-sm">Chat de prescri\u00e7\u00e3o habilitado</p><p className="text-xs text-muted-foreground">Permite que pacientes enviem prescri\u00e7\u00f5es</p></div>
